  ...
  return (
    <div className="auth-bg min-h-screen flex flex-col items-center justify-center p-5 relative">
      ...
      <div className="text-center mb-8 fade-up relative z-10">
        ...
        <p className="text-xs tracking-[0.25em] uppercase" style={{color:'#666'}}>All In Poker For Friends</p>
      </div>

      <div className="card-highlight w-full max-w-md p-6 fade-up relative z-10" style={{animationDelay:'0.1s'}}>
        <h2 className="font-display text-xl font-semibold mb-5 text-center tracking-wide">הצטרף לשולחן 🃏</h2>
        ...
        <div className="space-y-3">
          ...
          <input className="input" type="tel" placeholder="טלפון (אופציונלי)"
            value={phone} onChange={e => setPhone(e.target.value)} />
          <input className="input" type="email" placeholder="כתובת אימייל *"
            value={email} onChange={e => setEmail(e.target.value)} />
          <input className="input" type="password" placeholder="סיסמה (6+ תווים) *"
            value={password} onChange={e => setPassword(e.target.value)} />
          <button onClick={handleSignup} disabled={loading} className="btn btn-gold">
        ...
        <p className="text-center text-sm mt-5" style={{color:'#666'}}>
          יש לך חשבון?{' '}
          <Link href="/" style={{color:'var(--gold)',fontWeight:600}}>התחבר</Link>
        </p>
      </div>
    </div>
  )
